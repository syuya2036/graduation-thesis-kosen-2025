...
\begin{proof}
    ...
    これは、すべての$i$について条件を満たす必要があるため、$x_i$に対する上限は各$y_{i}$の最小値、すなわち$\min(y_{1,d}, y_{2,d}, \ldots, y_{K,d})$となる。
\end{proof}


% よって，区間$[l_1, r_1] \times [l_2, r_2] \times \ldots \times [l_D, r_D]$の和を求めるクエリ処理は，以下のアルゴリズムにより$O(2^D)$で行うことができる．$popcount(x)$は$x$を二進数表記した時の$1$の数を数える関数であり，計算量は$O(1)$を仮定する．
% \begin{algorithm}
